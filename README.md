...
| 伦敦金历史 K 线 | 东方财富 `push2his`（`122.XAU`） | 原生支持日/周/月，含重试 |
| 沪金历史 K 线 | 新浪期货日 K 接口 | 周/月 K 由日 K 聚合 |
...
│       ├── eastmoney.ts     # 伦敦金历史 K 线（东财 122.XAU，含重试）
│       ├── sinaHistory.ts   # 沪金历史日 K + 周/月聚合
│       ├── history.ts       # 按品种统一调度
│       └── brand.ts         # 品牌金价
├── src/                   # 渲染进程
...
- 数据源为公开接口，偶发限流/不稳定：伦敦金历史已加 3 次重试，所有 IPC handler 均有容错降级
